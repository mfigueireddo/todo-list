...
///   ASP.NET Core — validação automática do modelo, inferência da origem dos parâmetros, respostas
///   de erro padronizadas em ProblemDetails e exigência de roteamento por atributo.
/// 
...
public sealed class DatabaseHealthController : ControllerBase
{
    ...
    ///
    /// <summary>
    /// Recebe o <c>AppDbContext</c> resolvido pela injeção de dependência (registrado em
    /// <c>Program.cs</c> via <c>AddDbContext</c>) e o guarda para uso na verificação.
    /// </summary>
    ///
    ...
    /// <remarks>
    /// Assertivas de saída:
    /// O controller fica pronto para responder, com <c>_dbContext</c> apontando para a sessão de
    /// banco da requisição atual. Nenhuma conexão é aberta no construtor.
    /// </remarks>
    ///
    ...
    ///
    /// <summary>
    /// 1. Tenta abrir uma conexão com o banco através de <c>Database.CanConnectAsync</c>, que faz um
    ///    teste de conectividade leve (não cria tabelas nem lê dados de negócio).
    /// 
    /// 2. Traduz o resultado em uma resposta HTTP: disponível (200) ou indisponível (503).
    /// </summary>
    ///
    /// <returns>
    /// - Retorna HTTP 200 com <c>{ status = "ok", timeUtc }</c> quando a conexão com o banco é
    ///   estabelecida com sucesso.
    /// 
    /// - Retorna HTTP 503 (Service Unavailable) com <c>{ status = "unavailable", timeUtc }</c>
    ///   quando o banco não pode ser alcançado.
    /// </returns>
    ///
    /// <remarks>
    /// Atributos:
    ...
    ///
    /// Restrições:
    /// É um smoke test de conectividade: usa <c>CanConnectAsync</c> de propósito (em vez de uma
    /// query de negócio) para não depender de nenhum schema/tabela, que ainda não existem.
    /// </remarks>
    ///
    [HttpGet]
