...
///
/// <summary>
/// Objetivo: Expor os endpoints de autenticação e conta exigidos pelo docs/IDEA.md — cadastro, login,
/// visualização e exclusão da própria conta — emitindo o JWT que o frontend usa nas requisições protegidas.
///
/// Descrição:
/// 1. Recebe o <see cref="UserManager{TUser}"/> (operações de usuário do Identity), o <see cref="JwtTokenService"/> (emissão do token)
/// e o <see cref="AppDbContext"/> (limpeza de referências na exclusão de conta) por injeção de dependência.
/// 2. Cada action mapeia para uma URL própria sob a base <c>/auth</c>.
/// </summary>
///
/// <remarks>
/// Atributos:
/// - [ApiController]: marca como controller REST e ativa a validação automática do modelo (400 com os erros das anotações dos DTOs).
/// - [Route(Routes.Api.Auth)]: define a base de URL deste controller a partir da constante compartilhada (<c>"auth"</c>).
///
/// Restrições:
...
public sealed class AuthController : ControllerBase
{
    ...
    /// <summary>
    /// Descrição:
    /// 1. Cria um novo usuário no papel <see cref="AppRoles.User"/> a partir do <paramref name="request"/>.
    /// 2. Em sucesso, já autentica o usuário devolvendo um token (auto-login).
    /// </summary>
    ///
    ...
    /// <remarks>
    /// Atributos:
    /// - [HttpPost("register")]: mapeia para POST "/auth/register".
    /// - [FromBody] (em <paramref name="request"/>): o objeto vem do corpo JSON (inferido pelo [ApiController]).
    /// </remarks>
    ///
    [HttpPost("register")]
    ...
    /// <summary>
    /// Descrição:
    /// 1. Localiza o usuário pelo nome e verifica a senha contra o hash armazenado.
    /// 2. Em sucesso, devolve um token; em falha, responde 401 sem revelar se o erro foi no usuário ou na senha.
    /// </summary>
    ///
    ...
    /// <remarks>
    /// Atributos:
    /// - [HttpPost("login")]: mapeia para POST "/auth/login".
    /// - [FromBody] (em <paramref name="request"/>): o objeto vem do corpo JSON.
    ///
    /// Restrições:
    /// - A mensagem de erro é genérica de propósito (não distingue usuário inexistente de senha errada), para não facilitar enumeração de usuários.
    /// </remarks>
    ///
    [HttpPost("login")]
    ...
    /// <summary>
    /// Descrição:
    /// 1. Lê o usuário autenticado a partir do token e projeta seus dados de conta (sem senha).
    /// </summary>
    ///
    /// <returns>
    ...
    /// <remarks>
    /// Atributos:
    /// - [HttpGet("me")]: mapeia para GET "/auth/me".
    /// - [Authorize]: exige um JWT válido; sem ele o middleware responde 401 antes da action.
    /// </remarks>
    ///
    [HttpGet("me")]
    [Authorize]
    ...
    /// <summary>
    /// Descrição:
    /// 1. Lê o usuário autenticado.
    /// 2. Limpa (define como nulo) as referências de tarefas que apontam para ele (responsável e criador).
    /// 3. Exclui a conta.
    /// </summary>
    ///
    /// <returns>
    ...
    /// <remarks>
    /// Atributos:
    /// - [HttpDelete("me")]: mapeia para DELETE "/auth/me".
    /// - [Authorize]: exige um JWT válido.
    ///
    /// Restrições:
    /// - As tarefas NÃO são excluídas: por decisão do projeto, apenas as referências ao usuário são anuladas (FK com NoAction; ver AppDbContext).
    /// - Excluir um Admin é bloqueado para não remover o usuário semeado obrigatório; mesmo que removido, ele seria recriado no próximo startup.
    /// </remarks>
    ///
    [HttpDelete("me")]
    [Authorize]
    ...
    /// <summary>
    /// Descrição:
    /// 1. Obtém os papéis do usuário e gera o token JWT.
    /// 2. Monta o <see cref="AuthResponse"/> com o token e os dados básicos do usuário.
    /// </summary>
    ///
    ...
    /// <summary>
    /// Descrição:
    /// 1. Carrega as tarefas que referenciam o usuário como responsável ou criador.
    /// 2. Anula essas referências e persiste, mantendo as tarefas mas sem vínculo com a conta removida.
    /// </summary>
    ///
